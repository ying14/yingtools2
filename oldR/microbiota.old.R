...
#'
#' Extracts OTU data from phyloseq, including number of sequences and taxonomy. Uses the following reserved varnames: otu, sample, numseqs, pctseqs.
#' Note that phyloseq has a similar function, \code{psmelt}, but that takes longer. Also, calculates percent abundance.
#'
...
#' @param sample_data Logical, whether or not to join with sample_data. Default TRUE.
...
read.blastn.file <- function(tax.file,tax_table=TRUE,blastn.data=FALSE) {
  t <- fread(tax.file,colClasses=c("sallgi"="character","staxids"="character")) %>% tbl_df() %>%
    ...
    select(otu,evalue,pident,Kingdom,Phylum,Class,Order,Phylum,Class,Order,Family,Genus,Species,n.ties,blast.data,everything())
  if (tax_table) {
    if (blastn.data) {
      tt <- t %>% select(otu,Kingdom,Phylum,Class,Order,Phylum,Class,Order,Family,Genus,Species,blast.data)
    } else {
      tt <- t %>% select(otu,Kingdom,Phylum,Class,Order,Phylum,Class,Order,Family,Genus,Species)
    }
    return(tt)
  } else {
    return(t)
