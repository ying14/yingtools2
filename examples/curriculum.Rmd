...
#     tryCatch({
#       x <- get(.) %>% select_if(~(anyDuplicated(.)==0) & (is.character(.)|is.factor(.)))
#       ncol(x)>0
#     },error=function(e) {
#       NA
#     })
...
rows <- c(
  ...
  "Search for text using regular expressions"="`grepl(\"dog|cat\",sentences)` -or-\n`str_detect(sentences,\"dog|cat\")`",
...
rows <- c(
  ...
  "Subset the rows based on column criteria"="(cars that have >15 mpg and 6 cylinders)\n`filter(mtcars,mpg>15,cyl==6)` -or-\n`subset(mtcars,mpg>15 & cyl==6)` -or-\n`mtcars[mtcars$mpg>15 & mtcars$cyl==6,]`",
  ...
  # "Right join"="(keep all rows from instruments)\n`band_members %>% right_join(band_instruments,by=\"name\")`",
  # "Full join"="(keep all rows from band and instruments)\n`band_members %>% full_join(band_instruments,by=\"name\")`",
...
```{r ggplot_1, fig.height=3, fig.show="hide", fig.width=5, include=FALSE}

vec <- list(
"histogram"=expr({
  ggplot(diamonds,aes(x=price)) + geom_histogram()
}),
"histogram with custom fixed colors"=expr({
  ggplot(diamonds, aes(x=price)) + geom_histogram(color="black", fill="pink")
}),
"scatterplot"=expr({
...
```{r ggplot_2, fig.show = "hide", echo = FALSE, fig.height=3, fig.width=5,echo=FALSE}
